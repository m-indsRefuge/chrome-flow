...
test("reducers are immutable, bounded, class-separated, and preserve unknown fields", () => {
  ...
  assert.equal(contract.applyDomainMutation(source, "journal.append", { record: { text: "x" } }).workspace.journal.length, 1); assert.equal(contract.applyDomainMutation(source, "timeline.append", { record: { type: "x" } }).workspace.timeline.length, 1);
