...
async function reconcileActiveWorkspaceProjection(context = {}) {
  ...
  if (!latestWorkspace?.workspaceId || latestWorkspace.workspaceId !== baselineWorkspace.workspaceId) {
    const summary = createEmptySummary(
      ...
      triggers,
      startedAtMs,
      "aborted_workspace_identity_changed"
    );
  ...
  const summary = {
    schema: "workspace-projection-reconciliation-summary-v0.4",
  ...
  if (!mergeResult.changedRecordCount) {
    if (triggers.some(isStartupTrigger)) {
      await appendRuntimeDiagnostic(
        "info",
  ...
  latestWorkspace.timeline.push(
    ...createTransitionEvents(mergeResult.transitions, now, triggers),
    createReconciliationSummaryEvent(summary, now)
  );
  latestWorkspace.updatedAt = now;
  latestWorkspace.projectionReconciliation = {
    schema: "workspace-projection-reconciliation-v0.4",
    reconciledAt: now,
    triggers,
...
function resolveWorkspaceTabs(workspaceTabs, browserTabs) {
  ...
  for (const index of unresolvedIndexes) {
    ...
    if (url) unresolvedDemandByUrl.set(url, (unresolvedDemandByUrl.get(url) || 0) + 1);
  }
...
function createReconciliationSummaryEvent(summary, createdAt) {
  return {
    eventId: crypto.randomUUID(),
    type: "workspace_tabs_refreshed",
    message: "Workspace browser projection reconciled automatically: "
    ...
    reconciliationMode: "automatic_browser_projection",
    reconciliationTriggers: summary.triggers,
    foundCount: summary.openCount,
    missingCount: summary.missingCount,
    ambiguousCount: summary.ambiguousCount,
    recordsChanged: summary.recordsChanged
  };
