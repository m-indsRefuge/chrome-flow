...
async function reconcileActiveWorkspaceProjection(context = {}) {
  ...
  if (!baselineWorkspace?.workspaceId || baselineTabs.length === 0) {
    return createSummary(baselineWorkspace?.workspaceId || "", baselineTabs.length, 0, [], context.triggers, startedAtMs, "no_workspace_tabs");
  }
  ...
  if (!latestWorkspace?.workspaceId || latestWorkspace.workspaceId !== baselineWorkspace.workspaceId) {
    const summary = createSummary(baselineWorkspace.workspaceId, baselineTabs.length, 0, [], context.triggers, startedAtMs, "aborted_workspace_identity_changed");
  ...
  const summary = {
    schema: "workspace-projection-reconciliation-summary-v0.2",
  ...
  latestWorkspace.projectionReconciliation = {
    schema: "workspace-projection-reconciliation-v0.2",
    reconciledAt: now,
    triggers: context.triggers || [],
...
function resolveWorkspaceTabs(workspaceTabs, browserTabs) {
  ...
  return workspaceTabs.map((workspaceTab) => {
    ...
    if (exact) {
      ...
      return { workspaceTab, liveTab: exact, matchStatus: "exact_tab_id", candidateCount: 1 };
    }
    ...
    if (urlMatches.length === 1) {
      ...
      return { workspaceTab, liveTab: urlMatches[0], matchStatus: "single_url_fallback", candidateCount: 1 };
    }

    return {
      workspaceTab,
      liveTab: null,
      matchStatus: urlMatches.length > 1 ? "ambiguous_url_matches" : "not_found",
...
function createSummary(workspaceId, recordsExamined, recordsChanged, transitions, triggers, startedAtMs, status) {
  return {
    schema: "workspace-projection-reconciliation-summary-v0.2",
    workspaceId,
    recordsExamined,
