...
const WINDOW_SETTLE_DELAY_MS = 500;

let moveInProgress = false;

installStableNewWindowMoveOverride();

function installStableNewWindowMoveOverride() {
  ...
  button.addEventListener("click", (event) => {
    ...
    void moveWorkspaceTabsIntoNewWindowStable();
  }, true);
}

async function moveWorkspaceTabsIntoNewWindowStable() {
  if (moveInProgress) {
  ...
  try {
    ...
    await recordDiagnostic("info", "workspace_tabs_moved_to_new_window", "Moved workspace tabs into a new Chrome window using focus recovery and recreated workspace Chrome groups.", {
      newWindowId,
      tabIds: movedTabIds,
      workspaceTabIds,
      newWindowCreationMode: "primary_tab_new_window_focus_recovery",
    ...
    await recordDiagnostic("error", "workspace_tabs_new_window_failed", "Stable Move Workspace Into New Window failed.", {
      error: summarizeError(error)
    });

    if (workspace) {
      await addTimelineEvent(workspace, "workspace_tabs_new_window_failed", "Stable Move Workspace Into New Window failed before Chrome Flow could complete the tab move.", {
        error: summarizeError(error),
        newWindowCreationMode: "primary_tab_new_window_focus_recovery"
      });
    }
...
async function recordDiagnostic(level, action, message, details = {}) {
  try {
    ...
    console.warn("Chrome Flow stable new-window diagnostics failed:", error);
  }
}
